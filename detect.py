...
def run(
        ...
        source=ROOT / 'testing_data',  # file/dir/URL/glob, 0 for webcam
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        save_one_box(xyxy, imc, file=Path('cropped') / f'{p.stem}.jpg', BGR=True)
...
def parse_opt():
    ...
    parser.add_argument('--source', type=str, default=ROOT / 'testing_data', help='file/dir/URL/glob, 0 for webcam')
...
if __name__ == "__main__":
    ...
    '''f = 'dataset_test/NimishL_16.tif'
    
    img = load_img(f, target_size=(180, 180))
    img = img_to_array(img)
    img = np.expand_dims(img, axis=0)
    img = img/255
    
    y_pred = model.predict(img)
    classes_y=np.argmax(y_pred)
    
    print(classes_y)'''

    '''file = 'cropped/Aaryal_12.tif'
    ...
    print("Person Detected: ",y_pred)

#python detect.py --weights best.pt --img 640 --conf 0.35 --source testing_data
'''
